...
public class NormalMapRenderer implements Renderer {
	
	private NormalMapShader shader = new NormalMapShader();
	...
	public void render(Camera cam, List<Light> lights, boolean doFog, Vector4f clipPlane) {
		...
		for(TexturedModel texture : batches.keySet()) {
			...
			for(Entity ent : batches.get(texture)) {
				if(ent.getTexturedModel().getModel().shouldRender()) {
					prepareInstance(ent);
					GL11.glDrawElements(GL11.GL_TRIANGLES, ent.getTexturedModel().getModel().getVao().getIndexCount(), GL11.GL_UNSIGNED_INT, 0);
				}
			}
			unbindSkin(texture);
	...
	private void prepareInstance(Entity ent) {
		...
		//shader.loadTexturesDebug(new ArrayList<Texture>() {{add(ent.getModel().getTexture()); addAll(ent.getModel().getAdditionalTextures().getTextures()); addAll(additionalTextures);}});
	}
	
	private void unbindSkin(TexturedModel texture) {
		texture.getModel().getVao().unbindAttributes(0, 1, 2, 3);
	}
	...
	private void prepareSkin(TexturedModel texture) {
		...
		texture.getAdditionalTextures().bind(1);
		ShadowBuffers.shadowTexture().bind(2);
		Vao model = texture.getModel().getVao();
		model.bindAttributes(0, 1, 2, 3);
	}
